...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://crewgirl.online'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Crew Girl Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a fan-made wiki for the Netflix series Crew Girl, providing episode guides, cast profiles, and viewing resources.`)
  lines.push('')
